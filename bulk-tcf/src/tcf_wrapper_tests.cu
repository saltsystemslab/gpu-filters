...
int main(int argc, char** argv) {
	...
	using tcf_type = host_bulk_tcf<uint64_t, key_type>;
	...
	for (int batch = 0; batch< num_batches; batch++){
		...
		insert_diff[batch] = split_insert_timing(tcf, dev_keys, items_to_insert, misses);
		...
		//launch queries
		query_diff[batch] = bulk_query_timing(tcf, dev_keys, items_to_insert, misses);
		...
		bulk_query_timing(tcf, dev_keys, items_to_insert, misses);
