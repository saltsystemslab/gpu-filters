...
__global__ void insert_from_buffers_hashed_values(QF* qf,  uint64_t evenness, uint64_t * values){
	...
	uint64_t my_offset = buffers[idx] - buffers[0];
	...
	for (uint64_t i =0; i < my_count; i++){
		...
		int ret = qf_insert(qf, buffers[idx][i], values[my_offset+i], 1, QF_NO_LOCK | QF_KEY_IS_HASH);
...
__global__ void delete_from_buffers_hashed_vals(QF* qf, uint64_t evenness, uint64_t * vals){
	...
	uint64_t offset = buffers[idx] - buffers[0];


	//0 - my count for loop, working backwords should be faster?
	for (uint64_t i = my_count; i >=1; i--){

		int ret = qf_remove(qf, buffers[idx][i-1], vals[offset+i-1], 1, QF_NO_LOCK | QF_KEY_IS_HASH);

		//internal threadfence. Bad? actually seems to be fine
		//__threadfence();

		if (ret == -1) printf("Delete failed\n");
...
__device__ uint64_t point_query_count(QF* qf, uint64_t key, uint64_t value, uint8_t flags){
	...
	uint64_t ret = qf_count_key_value(qf, hash, value, QF_NO_LOCK | QF_KEY_IS_HASH);
...
__global__ void point_bulk_get(QF * qf, uint64_t * hashes, uint64_t nitems, uint64_t * counter){
	...
	if (point_query(qf, hashes[tid] % qf->metadata->range, query, QF_NO_LOCK) ==0){
...
__global__ void point_bulk_get_nocount(QF * qf, uint64_t * hashes, uint64_t nitems){
	...
	point_query(qf, hashes[tid] % qf->metadata->range, query, QF_NO_LOCK);
...
__host__ void bulk_insert_values(QF* qf, uint64_t nvals, uint64_t* keys, uint64_t * vals, uint8_t flags) {
	...
	thrust::sort_by_key(thrust::device, keys, keys+nvals, vals);
	...
	insert_from_buffers_hashed_values<<<(num_locks-1)/bulk_block_size+1, bulk_block_size>>>(qf, evenness, vals);
...
__host__ void bulk_delete_values(QF* qf, uint64_t nvals, uint64_t* keys, uint64_t * vals, uint8_t flags) {
	...
	delete_from_buffers_hashed_vals<<<(num_locks-1)/key_block_size+1, key_block_size>>>(qf, evenness, vals);
...
__global__ void bulk_get_exact_misses(QF * qf, uint64_t * keys, uint64_t * vals,  uint64_t nvals, uint64_t key_count, uint64_t * counter, uint8_t flags){
  ...
  if (tid >= nvals) return;
		...
		uint64_t count = qf_count_key_value(qf, keys[tid], vals[tid], 0);
...
__global__ void bulk_query_kernel(QF * qf, uint64_t * keys, bool * hits, uint64_t nvals){
...
__host__ void bulk_query(QF * qf, uint64_t * vals, bool * hits, uint64_t nvals){

	bulk_query_kernel<<<(nvals -1)/512+1, 512>>>(qf, vals, hits, nvals);
...
__host__ uint64_t bulk_get_exact_misses_wrapper(QF * qf, uint64_t * keys, uint64_t * vals, uint64_t nvals){
  ...
  bulk_get_exact_misses<<<(nvals-1)/512+1, 512>>>(qf, keys, vals, nvals, 1, misses, QF_NO_LOCK);
