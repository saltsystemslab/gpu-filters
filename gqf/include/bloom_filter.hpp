...
class bloom_filter
{
	...
	public:
		...
		__host__ __device__ bloom_filter()
			: bit_table_(0),
			salt_count_(0),
			table_size_(0),
			raw_table_size_(0),
			...
			desired_false_positive_probability_(p.false_positive_probability)
	{
		...
		__device__ bloom_filter(const bloom_filter& filter)
		{
		...
		__device__ inline bool operator!() const
		{
		...
		// {
		// 	std::fill_n(bit_table_,raw_table_size_,0x00);
		// 	//double check me for leak
		// 	cudaMemset(bit_table_, 0, (raw_table_size);
		// 	inserted_element_count_ = 0;



		__device__ inline virtual unsigned long long int size() const
		{
			return table_size_;
		}

		__device__ inline std::size_t element_count() const
		{
			return inserted_element_count_;
		}

		__device__ inline double effective_fpp() const
		{
			/*
Note:
...
*/
		...
		__device__ inline const cell_type* table() const
		{
			return bit_table_;
		}

		__device__ inline std::size_t hash_count()
		{
			return salt_.size();
		}
	...
	protected:
		...
		__host__ inline virtual void compute_indices(const bloom_type& hash, std::size_t& bit_index, std::size_t& bit) const
		{
			bit_index = hash % table_size_;
			bit = bit_index % bits_per_char;
		}

		__host__ void generate_unique_salt()
		{
			/*
Note:
...
class device_bloom_filter
{
	...
	{}
		...
		// 	std::fill_n(bit_table_,raw_table_size_,0x00);
		// 	//double check me for leak
		// 	cudaMemset(bit_table_, 0, (raw_table_size_-1)/8+1);
		// 	inserted_element_count_ = 0;
		// }

		__device__ inline void insert(const unsigned char* key_begin, const std::size_t& length)
		{
...
predicated/expected number of inserted elements.
*/
			return std::pow(1.0 - std::exp(-1.0 * salt_size * inserted_element_count_ / size()), 1.0 * salt_size);
		}
