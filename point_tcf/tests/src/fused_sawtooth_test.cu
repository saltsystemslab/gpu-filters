...
//#define key_bits 6
using del_TCF = poggers::data_structs::tcf_wrapper<uint64_t, uint8_t, 16, 0, 4, 16>::tcf;
...
__host__ void sawtooth_test(Sizing_Type * Initializer, int num_partitions, int num_rounds){
   ...
   for (int i = 0; i < num_rounds; i++){
      ...
      if (misses[5] != 0){
         ...
         printf("Batch %d: Failed to add %llu items in re-add\n", i, misses[5]);
...
int main(int argc, char** argv) {
   ...
   uint64_t second_nitems = nitems/100;

   //printf("Booting with %llu, %llu\n", nitems, second_nitems);
   ...
   for (int i = 0; i < 1; i++){
      ...
      //poggers::sizing::size_in_num_slots<1> bucket_size_2 (1ULL<<nbits);


      sawtooth_test<del_TCF, uint64_t, uint8_t>(&bucket_size_2, num_partitions, num_rounds);
